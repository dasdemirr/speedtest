...
Kullanım
	...
	•	Bağlanılan Sunucu: Bağlantı kurulan en iyi sunucu hakkında bilgi (sunucunun adı ve ülkesi).
	•	İndirme Hızı: İnternetten veri indirme hızınız, Mbps cinsinden.
	•	Yükleme Hızı: İnternete veri yükleme hızınız, Mbps cinsinden.
	•	Gecikme Süresi (Ping): Bağlantının yanıt süresi, milisaniye cinsinden.
	4.	Test sırasında bir hata oluşursa, kullanıcıya hata mesajı gösterilir.
...
Kodu Anlama
	...
	•	Speedtest: Speedtest kütüphanesi internet hızınızı test etmek için kullanılır. get_best_server(), download(), upload() ve ping gibi metodlarla hız ve gecikme ölçümleri yapılır.
	•	Streamlit: streamlit kütüphanesi, basit bir arayüz oluşturmak için kullanılır. Kullanıcı arayüzü bileşenleri (st.title, st.button, st.write, vb.) ile test başlatılır ve sonuçlar ekrana yazdırılır.
Hata Yönetimi
